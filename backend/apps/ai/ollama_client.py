...
Если ответ неполный или неверный — давай низкую оценку (1-4).
Если ответ частично верный — средняя оценка (5-7).
Если ответ полный и точный — высокая оценка (8-10).

Ответь ТОЛЬКО в формате JSON, без пояснений до и после:
{"score": 5, "feedback": "текст на русском", "strengths": ["пункт"], "improvements": ["пункт"]}"""

        try:
            response_text = self.generate(
                prompt,
                system=(
                    'Ты — технический интервьюер DevOps. '
                    'Отвечай ТОЛЬКО на русском языке. '
                    'Строго следуй JSON формату без дополнительного текста.'
                ),
            )
            if response_text:
